...
class ButtonNator(disnake.ui.View):
	...
	async def toggle(self, button, inter):
		...
		await inter.response.edit_message(embed = disnake.Embed(
				...
				description = f">>> **Теперь я буду банить приглашённых не верифицированных ботов.**",
	...
	async def disable(self, button, inter):
		...
		await inter.response.edit_message(embed = disnake.Embed(
				...
				description = f">>> **Теперь я не стану банить не верифицированных ботов.**",
				color = disnake.Colour.dark_green()
			), view = None)
		self.data.commit()
		button.disabled = True
	...
	async def on_member_join(self, member):
		try:
			...
			if member.bot and not member.public_flags.verified_bot and entry.user.id != member.guild.owner_id and vv == member.guild.id:
				try:
					await member.ban(reason = 'Anti Bot Add')
				except:
					pass
		except IndexError:
			return
	...
	async def antibot(self, ctx):
		...
		if inter.component.custom_id == 'toggle':
			...
			await inter.response.edit_message(embed = disnake.Embed(
				...
				description = f">>> **Теперь никто не сможет добавить не верифицированных ботов.**",
			...
			await inter.response.edit_message(embed = disnake.Embed(
				...
				description = f">>> **Данная функция больше не будет работать на вашем сервере, пока вы её не включите.**",

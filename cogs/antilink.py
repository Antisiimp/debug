	...
	async def on_message(self, message):
		try:
			...
			if(guild == message.guild.id and not message.author.guild_permissions.administrator and not chan):
				if(re.search("https://", message.content) or re.search("http://", message.content) or re.search("discord.gg/", message.content) or re.search("youtube.com/", message.content) ):
					await message.delete()
					await message.author.ban(reason = "1.4")
		except IndexError:
			return

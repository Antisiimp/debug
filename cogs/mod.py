...
class Program(commands.Cog):
	...
	@commands.command(aliases = ['сервер', 'serverinfo', 'si', 'infoserver', 'server-info'])
...
Уровень 2mFa: `{ctx.guild.mfa_level}`
Бустов: `{ctx.guild.premium_subscription_count}`**""",
	...
	@commands.command(aliases = ['leave-ban', 'lb'])
	...
	async def mute(self, ctx, member: disnake.Member = None, reason=None):
		...
		if member:
			...
			await ctx.send(embed=disnake.Embed(title = ':x: | Мут',
					description=f">>> **Пользователь `{ctx.author.name}` выдал мут `{member.name}` на `{inter.component.label}`**",
					color=disnake.Colour.green()))
		else:
	...
	async def warn(self, ctx, member: disnake.User = None, reason=None):
		if member:
			...
			await ctx.send(embed = disnake.Embed(title = f"{member}",
				description = f">>> **Пользователь `{ctx.author.name}`выдал предупреждение `{member.name}`**",
				color = disnake.Colour(self.color)
				))
	...
	async def unwarn(self, ctx,  member: disnake.User):
		if member:
			...
			if ifable:
				...
				await ctx.send(embed=disnake.Embed(title = ":gear: | Предупреждение",

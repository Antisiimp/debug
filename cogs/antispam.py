...
class Worker(commands.Cog):
	...
	@commands.command(aliases = ['ignore', 'ignore-channel'])
	...
	@commands.command(aliases = ['wld', 'whitelist'])
	...
	@commands.command(aliases = ['antilinks', 'anti-link', 'anti-links'])
	@commands.cooldown(1, 30, commands.BucketType.default)
	@commands.has_permissions(send_messages = True)
	...
	@commands.command(aliases = ['arb', 'auto-reg-ban', 'auto-regb', 'autoregb', 'autoreg-ban', 'autoreg-b'])
	@commands.cooldown(1, 30, commands.BucketType.default)
	@commands.has_permissions(administrator = True)
	async def auto_reg_ban(self, ctx, days:int = 30):
		...
		await ctx.send(embed = disnake.Embed(title = ":gear: | Включено",
			description = f'>>> **Функция включена. Теперь, если на ваш сервер зайдёт пользователь, регистрация которого меньше `{days}` дней, то он получит бан**',
			color = disnake.Colour(self.color)))
		self.data.commit()
